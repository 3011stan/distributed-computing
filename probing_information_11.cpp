...
void probing_process(int &int_sum, float &float_sum) {
  ...
  if (tag == 0) {
    int receiv_int;
    MPI_Recv(&receiv_int, 1, MPI_INT, source, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    int_sum += receiv_int;
  } else if (tag == 1) {
    float receiv_float;
    MPI_Recv(&receiv_float, 1, MPI_FLOAT, source, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    float_sum += receiv_float;
  }
}