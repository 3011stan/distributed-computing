void play_non_blocking_scenario() {
  ...
  if (rank == 0) {
    ...
    // 1- Initialise the non-blocking send to process 1 (tag 0)
    ...
    // 4- Prepare another request for process 1 with a different tag (tag 1)
    ...
    // 7- Initialise the non-blocking receive from process 0 (tag 0)
    ...
    // 9- Initialise another non-blocking receive (tag 1)
